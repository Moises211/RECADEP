...
export class UsersService {
  ...
  sincronizarUsuario(payload: Users): Observable<Users> {
    return this.http
      .post<Users>(`${this.apiUrl}/sync`, payload)
      .pipe(catchError((err) => this.handleError(err, 'sincronizarUsuario')));
  }

  getUserByEmail(email: string): Observable<Users> {
    return this.http
      .get<Users>(`${this.apiUrl}/by-email?email=${email}`)
      .pipe(catchError((err) => this.handleError(err, 'getUserByEmail')));
  }
}
